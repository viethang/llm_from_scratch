...
def get_batches(data, split, batch_size, context_window, config=MASTER_CONFIG):
    ...
    ix = torch.randint(0, batch_data.size(
        0) - context_window - 1, (batch_size,))
...
MASTER_CONFIG.update({
    ...
    # Number of characters in each input (x) and target (y) sequence of each batch
    'context_window': 16
})
